...
def create_ticket(db:Session,subject:str,body:str,created_by:int)->Ticket:
    db_ticket=Ticket(
        subject=subject,
        body=body,
...
def get_created_ticket(db:Session,ticket_sub:str)-> Ticket | None:
    return db.query(Ticket).filter(Ticket.subject==ticket_sub).first()
